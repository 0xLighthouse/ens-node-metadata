...
uniswap.eth
  ...
  │     └── class        = "Contract"
  │     └── alias        = "UniswapV3Factory"
  │     └── description  = "Deploys new Uniswap V3 pools"
  │     └── category     = "defi"
  │
  └── v3-router.uniswap.eth
        └── class        = "Contract"
        └── alias        = "SwapRouter"
        └── description  = "Routes swaps across Uniswap V3 pools"
        └── category     = "defi"
```

Each subnode address resolves to the actual on-chain contract or wallet. The parent ENS name provides the attestation: the organisation that controls `uniswap.eth` is the one asserting these subnodes belong to it.
