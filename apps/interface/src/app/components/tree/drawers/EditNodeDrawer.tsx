...
export function EditNodeDrawer() {
  ...
  const {
    ...
    pendingNodeSwitch,
    clearPendingNodeSwitch,
    openEditDrawer,
  } = useTreeEditStore()

  const { setSelectedNode: setTreeSelectedNode } = useTreeControlsStore()
  ...
  // Warning dialog state for pending-changes guard
  const [warningOpen, setWarningOpen] = useState(false)
  const [warningType, setWarningType] = useState<'close' | 'nodeSwitch'>('close')
  ...
  // Handle pending node switch (guarded by unsaved-changes check)
  useEffect(() => {
    if (pendingNodeSwitch === null) return

    if (hasChanges) {
      // Store pending target and show warning
      setPendingNodeId(pendingNodeSwitch)
      setWarningType('nodeSwitch')
      setWarningOpen(true)
      clearPendingNodeSwitch()
    } else {
      // No unsaved changes — switch immediately
      openEditDrawer(pendingNodeSwitch)
      clearPendingNodeSwitch()
      resetEditor()
    }
  ...
  /** Request close — intercepts when there are unsaved changes */
  const handleCloseRequest = () => {
    if (hasChanges) {
      setWarningType('close')
      setWarningOpen(true)
    } else {
      closeEditDrawer()
      resetEditor()
  ...
  /** Called when the user confirms "Discard changes" in the warning dialog */
  const handleWarningDiscard = () => {
    setWarningOpen(false)
    if (warningType === 'nodeSwitch' && pendingNodeId) {
      resetEditor()
      openEditDrawer(pendingNodeId)
      setTreeSelectedNode(pendingNodeId)
  ...
  /** Called when the user chooses "Continue editing" in the warning dialog */
  ...
  return (
    <>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You have unsaved changes. Do you want to discard them and continue, or keep editing?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="sm:justify-start">
            ...
            <Button type="button" onClick={handleWarningDiscard}>
              Discard changes
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Drawer.Root
        ...
        onOpenChange={(open) => {
          if (!open) {
            // Outside-click or Escape — guard with pending changes check
            if (hasChanges) {
              // Ignore — user must use the X button or confirm discard
              return
            }
            closeEditDrawer()
            resetEditor()
