...
interface BondsState {
  ...
  bondsOwned: Lock[]
  bondsAvailable: Lock[]
  fetchingBondsAvailable: boolean
...
export const useBondsStore = create<BondsState>((set, get) => ({
  ...
  fetchInitiativeLocks: async (initiativeId: string) => {
    ...
    // const state = get()
    // if (
    //   state.isFetchingInitiativeLocks ||
    //   state.isInitiativeLocksInitialized ||
    //   state.isFetchingInitiativeLocks
    // )
    //   return
  ...
  fetchBondsAvailable: async (hookAddress: string) => {
    ...
    // // Prevent duplicate fetches
    // if (get().fetchingBondsAvailable || get().isBondsAvailableInitalized) return

    // try {
    //   set({ fetchingBondsAvailable: true })
