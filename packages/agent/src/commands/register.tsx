...
export default function Register({ args: [ensName, payloadFile], options }: Props) {
  const { exit } = useApp()
  ...
  React.useEffect(() => {
    async function run() {
      ...
      if (!options.broadcast) {
        // Dry run — print what would be set
        const lines = [
      ...
      setState({
        status: 'working',
        message: `Setting ${texts.length} text records on ${ensName}…`,
      })
      try {
